...
class LSTMRNN(object):
	...
	def add_cell(self):
		lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(self.cell_size, forget_bias = 1. , state_is_tuple=True)
	...
	def add_output_layer(self):
		...
		with tf.name_scope('Wx_plus_b'):
			self.prediction = tf.matmul(l_out_x, Ws_out) + bs_out
...
if __name__ =='__main__':
	...
	for i in range(200):
		...
		_, cost, state , prediction = sess.run(
			[model.train_op, model.cost, model.cell_final_state, model.prediction],
			feed_dict = feed_dict
			)
